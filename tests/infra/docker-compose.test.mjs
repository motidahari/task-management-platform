...
const composeSource = readFileSync(`${repoRoot}docker-compose.yml`, 'utf8');
...
describe('docker-compose', () => {
  describe('services', () => {
    ...
    it('spells out no credential anywhere — every secret comes from the environment', () => {
      const interpolationFree = composeSource.replace(/\$\{[^}]*\}/g, '');
      assert.doesNotMatch(interpolationFree, /(password|passwd|pwd)[^\S\n]*[:=][^\S\n]*\S/i);
      assert.doesNotMatch(interpolationFree, /:\/\/[^\s/@]*:[^\s/@]+@/);
    });
  ...
  describe('postgres', () => {
    ...
    it('demands its credentials from the environment instead of defaulting them', () => {
      const { POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB } = service('postgres').environment;
      for (const value of [POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB]) {
  ...
  describe('backend', () => {
    ...
    it('carries no fallback credentials in its database URL', () => {
      assert.doesNotMatch(service('backend').environment.DB_URL, /:-/);
    });
