...
The `SeedUsers` migration inserts four users. Their ids are generated by the
database per environment, so they differ between local, CI and staging — never
hard-code one. `email` is the stable identity (the seed is idempotent on it and
safe to re-run), and `GET /api/v1/users` is how a client learns the ids.
