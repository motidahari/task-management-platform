...
describe('createEventBus', () => {
  let testBus: ReturnType<typeof createEventBus<TestEvents>>;

  beforeEach(() => {
    vi.clearAllMocks();
    testBus = createEventBus<TestEvents>();
  });

  describe('Given:a handler registered for an event', () => {
  ...
  describe('Given:an event emitted with no registered handlers', () => {
    it('should not throw', () => {
      expect(() => testBus.emit('ping', { count: 1 })).not.toThrow();
    });
  });

  describe('Given:a void-payload event', () => {
  ...
  describe('Given:a handler removed via the unsubscribe function on() returns', () => {
  ...
  describe('Given:a handler removed via off()', () => {
    it('should stop receiving further emissions', () => {
      const handler = vi.fn();
